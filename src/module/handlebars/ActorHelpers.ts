...
export const registerActorHelpers = () => {
    ...
    */
    Handlebars.registerHelper('calcModificationCategorySlots', (items: [SR5Item], modificationCategory: VehicleModificationCategoryType): number => {        
        if (!Array.isArray(items) || !items.length) { return 0 }        
        let slotSum = 0;
        
        for (const item of items) {
            if (item.system.vehicleMod?.modification_category === modificationCategory) {
                ...
                slotSum += (item.system.vehicleMod?.slots || 0) * quantity;
            }
        }
    ...
    */
    Handlebars.registerHelper('calcModPointSlots', (items: [SR5Item]): number => {
        ...
        const slotSum = items.reduce((arr, item) => {            
            if (item.system.type == dronestring) { return arr += item.system.vehicleMod?.slots ? item.system.vehicleMod?.slots : 0 } else { return arr };            
        }, 0)
